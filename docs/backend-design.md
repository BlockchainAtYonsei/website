...
- **아티클 — 실데이터.** Notion "아티클" DB(`NOTION_DB_ARTICLES`)와 붙어
  뉴스처럼 sync가 돈다. `상태` 어휘(완료/진행 중/시작 전)는 `schema.ts`에서
  발행/초안으로 매핑되고, `작성자`는 `팀원 소개` DB 관계형을 로스터와 이름
  매칭한다. mock 시드는 제거됨.

## 8. 남은 것

1. **아바타** — 사진이 모이면 뉴스 이미지와 같은 재호스팅 경로 사용.
2. **후순위**: webhook, 검색(tsvector).
```
