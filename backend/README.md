...
공백 무시로 대조) · Medium URL(url) · 커버(files) · 커버 출처(text).

**커버는 대개 비워둬도 된다.** 본문 끝에 `참고자료`(또는 출처/References)
섹션을 두면 sync가 **첫 번째 참고자료의 링크 미리보기 이미지**를 가져와
카드 그림으로 쓰고, `사진: [참고자료 제목](링크)`으로 출처를 밑에 찍는다 —
뉴스가 원문 링크에서 og:image를 긁어오는 것과 같은 규칙이다. 미리보기
이미지가 없는 사이트면 경고를 남기고 생성 아트로 돌아간다.

직접 고른 그림을 쓰고 싶을 때만 커버를 채운다. 업로드해도 되고 이미지 URL을
붙여넣어도 된다 — 업로드본은 sync가 버킷으로 복사한다(Notion 업로드 URL은 한
시간 뒤 죽는다). **남의 사진이면 커버 출처를 반드시 채운다**: 재사용을
허용하는 라이선스는 대부분 출처 표기가 조건이고, 이 값이 홈 고정 슬롯과 기사
헤더의 사진 밑에 그대로 찍힌다. `사진: 촬영자 / 출처 (라이선스)` 형태로 쓰고
출처 단어에 **노션에서 하이퍼링크를 걸면** 사이트에서도 링크로 나온다. 커버만
있고 출처가 비면 경고를 남긴다(발행은 된다 — 직접 만든 그림은 출처가 없으니까).

우선순위: 본문 첫 이미지 > 커버 > 첫 참고자료의 미리보기 > 생성 아트.
